  ...
  <div :class="['bg-light', { fixed: showRates }]">
    ...
    <Presence>
      <Motion
        ...
        :animate="{ scaleY: [0.6, 1], opacity: [0, 1] }"
        :exit="{ scaleY: [0.6], opacity: 0 }"
        :transition="{ duration: 0.25, easing: 'ease-in' }"
    ...
    <Presence>
      <Motion
        :initial="{
          transform: 'scaleY(0.2)  translateY(-50%) translateX(-50%)',
          opacity: 0,
        }"
        :animate="{
          transform: [
            'scaleY(0.2) translateY(-50%) translateX(-50%)',
            'scaleY(1) translateY(-50%) translateX(-50%)',
          ],
          opacity: [0, 1],
        }"
        :exit="{
        ...
        :transition="{ duration: 0.3, easing: 'ease-out', delay: 0.1 }"
        v-if="showRates"
          ...
          <div class="flex flex-col space-y-16 w-80 mx-auto">
            ...
            <div class="flex justify-between">
              ...
              <p class="font-gilmer text-sm">
                ...
                {{
                  formatPrecisionNumber(ratesInformation.targetCurrencyAmount)
                }}
              </p>
            </div>
          </div>
    ...
    <main class="flex flex-col relative">
      ...
      <section class="pt-10 pb-36 relative" id="hero">
        <div
          class="container flex flex-col lg:grid grid-cols-2 gap-x-32 items-center"
        >
          <div
            class="flex flex-col space-y-6 mt-16 lg:mt-0 text-center lg:text-left"
          >
            <h1 class="font-title font-black text-3xl lg:text-5xl">
              Compare Foreign Exchange Charges With Other Providers
            </h1>
            <p class="font-gilmer lg:text-xl">
              You can now compate the rates we charge for international
              transactions on Grip with rates offered by other providers.
            </p>
            <div class="font-gilmer mt-5 lg:text-xl">
              ...
              <ul class="list-disc list-inside mt-2 text-left">
                <li class="list-item">
                  Rates vary depending on your subscription tier and how much of
                  your spending limit has been utilized.
                </li>
          ...
          <form
            class="grid grid-cols-1 gap-y-11 mt-16 lg-mt-0 w-full lg:w-auto"
          ...
          >
            ...
            <div class="grid grid-cols-1 gap-y-5">
              <label
                for="base-currency"
                class="block font-medium text-gray-700 font-gilmer"
                >Base Currency</label
              >
              <div class="mt-1">
                <select
                ...
                >
                  ...
                  <option
                    ...
                    v-for="(currency, index) in [...supportedCurrencies as NonNullable<typeof supportedCurrencies>]?.filter(x => x.value != targetCurrency)"
                    :key="index"
                  >
                    {{ currency.name }}
                  </option>
            ...
            <div class="grid grid-cols-1 gap-y-5">
              ...
              <div class="mt-1">
                ...
                >
                  ...
                  <option
                    :value="currency.value"
                    v-for="(currency, index) in [...supportedCurrencies as NonNullable<typeof supportedCurrencies>].filter(
                      (x) => x.value != baseCurrency
                    )"
                    :key="index"
                  >
                    {{ currency.name }}
            ...
            <div class="grid grid-cols-1 gap-y-5">
              <label
                ...
                class="block font-medium text-gray-700 font-gilmer"
                >Amount
                <span v-if="targetCurrency"
                  >({{ getCurrencyInformation(targetCurrency)?.name }})</span
                ></label
              >
              <div class="mt-1">
                <input
...
const baseCurrency = ref<number | null>(null);
const targetCurrency = ref<number | null>(null);
const amount = ref("");
...
const calculateEchangeRate = async () => {
  ...
  const {
    ...
    pending,
    refresh,
  } = await useFetch<FxConversionResponse["data"]>(`/api/fx`, {
    method: "POST",
    body: JSON.stringify({
...
const getCurrencyInformation = (currencyId: string | number) => {
  return supportedCurrencies?.value?.find(
    (x) => x.value.toString() == currencyId ?? 0
  );
};
